if __name__ == "__main__":
    ...
    class Model:
        ...
        def run_sim(self, model_param = None, x0=None): #takes in canidate parameters then solves the ode
            ...
            solver = nbkode.RungeKutta45(self.sys_fun, t0=self.ts[0], y0=x0, params=([model_param]))
            result = solver.run(self.ts)
        ...
        def log_likelihood(self, theta_new): #how good is this canidate parameter fitting my data (maximize it)
            ...
            model_param = np.array(model_param, dtype=np.float64)
    ...
    def mod2_sys(t, y, theta): #makes a new function
        ...
        #dx2dt = (k2 * (x1**2)) - (k1 * x2)
        #print(dx1dt, dx2dt)
        temp[0] = (2 * k1 * x2) - (k2 * (x1**2)) - (k3 * x1 * x2) - (k4 * x1)
        temp[1] = (k2 * (x1**2)) - (k1 * x2)
        return temp
