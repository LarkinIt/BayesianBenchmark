...
class Model:
    ...
    def change_and_run(self, model_param, x0):
        ...
        t_span = (self.ts[0], self.ts[-1]) # make it the length of the data 
        trajs = rr.simulate(t_span[0], t_span[1], len(self.ts))
        # trajs = rr.simulate(t_span[0], t_span[1], int(t_span[1]+1)) # runs for mRNA model
        # trajs = rr.simulate(t_span[0], t_span[1], 17) # only goes to t=16
        # trajs = rr.simulate(t_span[0], t_span[1], int(t_span[1]*100+1)) # caroline's original
        # trajs = rr.simulate(t_span[0], t_span[1], 21) #int(t_span[1]+1)) # changed for mm model 
        return trajs    
